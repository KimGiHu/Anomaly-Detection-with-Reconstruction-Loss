...
# numpy 파일로 저장할 경로 설정
anomaly_file_path = './data/anomaly_gausssian_sampel200_multi_variance.npy'
...
# # 정상신호 테스트 결과 저장하기
# with torch.no_grad():
#     pbar = tqdm(dataloader_anomaly_1st, total=len(dataloader_anomaly_1st), ncols=100)
...
#     # plt.grid(True)
#     plt.savefig(args.figure_path+'standard_ver1.png', dpi=600)
...
# # 이상신호 추론 결과 저장하기
# with torch.no_grad():
#     pbar = tqdm(dataloader_anomaly_origin, total=len(dataloader_anomaly_origin), ncols=100)
#     for x_input, label in pbar :
#         x_input, label = x_input.to(device), label.to(device)
#         x_recon, _, _ = model_VAE(x_input, label)
...
#     # plt.title('Original Signal vs. Reconstruction Signal')
#     # plt.grid(True)
#     plt.savefig(args.figure_path+'Anomaly.png', dpi=600)
...
sns.boxplot(x='Type', y='MSE_anomaly_0',data=data_anomaly_0, label='0.001',color='orange',flierprops=dict(marker='None'))
sns.boxplot(x='Type', y='MSE_anomaly_1st',data=data_anomaly_1st, label='0.01',color='orange',flierprops=dict(marker='None'))
...
sns.boxplot(x='Type', y='MSE_anomaly_10th',data=data_anomaly_10th, label='0.10',color='blue',flierprops=dict(marker='None'))
