...
parser.add_argument('--figure_path', type=str,
                    default='./JKPS/plot/', help='Enter the dataset path')
...
# 정상신호 테스트 결과 저장하기
with torch.no_grad():
    pbar = tqdm(dataloader_anomaly_2nd, total=len(dataloader_anomaly_2nd), ncols=100)
    ...
    input_sig = origin[0,:,:]
    # reconstruction_sig = regenerate[0,:,:]
    tmp = data_test[100+1472:100+1472+1,:,:].transpose(0,2,1)
    for i in range(0, num_channels):
        if i==0:
            tmp[:,:,i] = load_channel1.inverse_transform(tmp[:,:,i].reshape(-1,1)).reshape(tmp[:,:,i].shape)
        if i==1:
            tmp[:,:,i] = load_channel2.inverse_transform(tmp[:,:,i].reshape(-1,1)).reshape(tmp[:,:,i].shape)
        if i==2:
            tmp[:,:,i] = load_channel3.inverse_transform(tmp[:,:,i].reshape(-1,1)).reshape(tmp[:,:,i].shape)
        if i==3:
            tmp[:,:,i] = load_channel4.inverse_transform(tmp[:,:,i].reshape(-1,1)).reshape(tmp[:,:,i].shape)
    reconstruction_sig = tmp.reshape(tmp.shape[1],tmp.shape[2])
    print(reconstruction_sig.shape)
    ...
    for i in range(num_channels):
        if i==3 :
            # plot할 파형 정하기
            plt.plot(time, input_sig[:,i], color='blue')
            plt.plot(time, reconstruction_sig[:,i], color='red')
            # 두 곡선 사이를 색으로 채우기
            plt.fill_between(time, input_sig[:,i], reconstruction_sig[:,i], where=(reconstruction_sig[:,i] > input_sig[:,i]), 
                            color="yellow", alpha=0.5, label="Error (Above)")
            plt.fill_between(time, input_sig[:,i], reconstruction_sig[:,i], where=(reconstruction_sig[:,i] <= input_sig[:,i]), 
                            color="yellow", alpha=0.5, label="Error (Below)")
    ...
    # plt.grid(True)
    plt.xlim(500,1700)
    plt.savefig(args.figure_path+'difference anomaly generated_channel4.png', dpi=600)
...
# boxplot의 중앙값/Q1/Q3 계산하기
def boxplot_factor (data, file_name):
    print("=========================") 
    ...
    print(f'상한 값 : {upper_bound}')

    # 상한 및 하한 기준으로 데이터 필터링하기
    data = np.array(data)
    filtered_data = data[(data>=lower_bound)&(data<=upper_bound)]
    
    # 필터링된 데이터 중앙값 및 표준편차 계산
    median =  np.median(filtered_data)
    std = np.std(filtered_data)

    print(f'필터링 된 데이터 중앙값 : {median}')
    print(f'필터링 된 데이터 표준편차 : {std}')
    print("=========================")
    ...
    results = {
        ...
        "Filtered Std Dev" : [std]
    }
    df = pd.DataFrame(results)
    df.to_csv(file_name, index=False, mode='a', header=not pd.io.common.file_exists(file_name))


# print("가우시안 노이즈 표준편차 0.001")
# boxplot_factor(mse_anomaly_0, args.figure_path+'std ver0.csv')
# print("가우시안 노이즈 표준편차 0.01")
# boxplot_factor(mse_anomaly_1st, args.figure_path+'std ver1.csv')
# print("가우시안 노이즈 표준편차 0.02")
# boxplot_factor(mse_anomaly_2nd, args.figure_path+'std ver2.csv')
# print("가우시안 노이즈 표준편차 0.03")
# boxplot_factor(mse_anomaly_3rd, args.figure_path+'std ver3.csv')
# print("가우시안 노이즈 표준편차 0.04")
# boxplot_factor(mse_anomaly_4th, args.figure_path+'std ver4.csv')
# print("가우시안 노이즈 표준편차 0.05")
# boxplot_factor(mse_anomaly_5th, args.figure_path+'std ver5.csv')
# print("가우시안 노이즈 표준편차 0.06")
# boxplot_factor(mse_anomaly_6th, args.figure_path+'std ver6.csv')
# print("가우시안 노이즈 표준편차 0.07")
# boxplot_factor(mse_anomaly_7th, args.figure_path+'std ver7.csv')
# print("가우시안 노이즈 표준편차 0.08")
# boxplot_factor(mse_anomaly_8th, args.figure_path+'std ver8.csv')
# print("가우시안 노이즈 표준편차 0.09")
# boxplot_factor(mse_anomaly_9th, args.figure_path+'std ver9.csv')
# print("가우시안 노이즈 표준편차 0.10")
# boxplot_factor(mse_anomaly_10th, args.figure_path+'std ver10.csv')
...
# sns.kdeplot(np.log1p(resotred_z_anomaly), fill=True, label='Pre-fault(1-$\sigma$ = 0.01)', cut=0, color='red', common_norm=False, bw_adjust=0.25) # 테스트 데이터
# sns.kdeplot(np.log1p(resotred_z_anomaly_10th), fill=True, label='Pre-fault(1-$\sigma$ = 0.10)', cut=0, color='blue', common_norm=False, bw_adjust=0.25) # 테스트 데이터
sns.kdeplot((mse_test), fill=True, label='Normal', cut=0, color='green', common_norm=False,bw_adjust=0.25) # 테스트 정상 데이터
sns.kdeplot((mse_anomaly_1st), fill=True, label='Pre-fault(1-$\sigma$ = 0.01)', cut=0, color='orange', common_norm=False, bw_adjust=0.25) # 표준편차 0.01
sns.kdeplot((mse_anomaly_2nd), fill=True, label='Pre-fault(1-$\sigma$ = 0.02)', cut=0, color='pink', common_norm=False, bw_adjust=0.25) # 표준편차 0.02
sns.kdeplot((mse_anomaly_5th), fill=True, label='Pre-fault(1-$\sigma$ = 0.05)', cut=0, color='red', common_norm=False, bw_adjust=0.25) # 표준편차 0.05
sns.kdeplot((mse_anomaly_10th), fill=True, label='Pre-fault(1-$\sigma$ = 0.10)', cut=0, color='blue', common_norm=False, bw_adjust=0.25) # 표준편차 0.10
...
# 박스 그림 그리기
sns.boxplot(x='Type', y='MSE_train',data=data_train, label='Train Dataset',color='g',flierprops=dict(marker='None'),legend=False)
sns.boxplot(x='Type', y='MSE_test',data=data_test, label='Test Dataset',color='b',flierprops=dict(marker='None'),legend=False)
sns.boxplot(x='Type', y='MSE_anomaly_0',data=data_anomaly_0, label='0.001',color='orange',flierprops=dict(marker='None'),legend=False)
sns.boxplot(x='Type', y='MSE_anomaly_1st',data=data_anomaly_1st, label='0.01',color='orange',flierprops=dict(marker='None'),legend=False)
sns.boxplot(x='Type', y='MSE_anomaly_2nd',data=data_anomaly_2nd, label='0.02',flierprops=dict(marker='None'),legend=False)
sns.boxplot(x='Type', y='MSE_anomaly_3rd',data=data_anomaly_3rd, label='0.03',flierprops=dict(marker='None'),legend=False)
sns.boxplot(x='Type', y='MSE_anomaly_4th',data=data_anomaly_4th, label='0.04',flierprops=dict(marker='None'),legend=False)
sns.boxplot(x='Type', y='MSE_anomaly_5th',data=data_anomaly_5th, label='0.05',flierprops=dict(marker='None'),legend=False)
sns.boxplot(x='Type', y='MSE_anomaly_6th',data=data_anomaly_6th, label='0.06',flierprops=dict(marker='None'),legend=False)
sns.boxplot(x='Type', y='MSE_anomaly_7th',data=data_anomaly_7th, label='0.07',flierprops=dict(marker='None'),legend=False)
sns.boxplot(x='Type', y='MSE_anomaly_8th',data=data_anomaly_8th, label='0.08',flierprops=dict(marker='None'),legend=False)
sns.boxplot(x='Type', y='MSE_anomaly_9th',data=data_anomaly_9th, label='0.09',flierprops=dict(marker='None'),legend=False)
sns.boxplot(x='Type', y='MSE_anomaly_10th',data=data_anomaly_10th, label='0.10',color='blue',flierprops=dict(marker='None'),legend=False)
