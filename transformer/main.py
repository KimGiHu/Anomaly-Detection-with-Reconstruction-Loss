...
class VAE(nn.Module):
    def __init__(self, input_dim, hidden_dim, latent_dim, condition_dim, num_heads=4, num_layers=3, dropout_prob=0.2):
        ...
        # 커스텀 인코더 레이어 정의
        encoder_layer = CustomTransformerEncoderLayer(d_model=hidden_dim, nhead=num_heads, dropout=dropout_prob)
        self.encoder = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)
        self.fc_mu_logvar = nn.Linear(hidden_dim + condition_dim, latent_dim * 2)
        
        # 디코더 정의
        self.fc_latent_to_input = nn.Linear(latent_dim + condition_dim, hidden_dim)
        decoder_layer = nn.TransformerEncoderLayer(d_model=hidden_dim, nhead=num_heads, dropout=dropout_prob)
...
criterion = nn.MSELoss()  # 예시로 MSE 손실 사용
...
model_VAE.load_state_dict(torch.load('./model/1022/M02_batch_size_64.pth'))
